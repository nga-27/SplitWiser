""" UI utility functions for cmd_app """
...
def terminal_pretty_print_spacer(line_str: str, max_tabs: int = 5,
                                 date: Union[None, str] = None) -> str:
    """ returns tabs! (and sometimes dates) """
...
def format_transactions(transactions: dict, table_name: str, return_str: bool = False,
                        color: Union[str, None] = None) -> Union[None, str]:
    """format_transactions

    Creates a command window-friendly table of transactions

    Args:
        transactions (dict): transaction dict
        table_name (str): table name
        return_str (bool, optional): if the print object should be returned. Defaults to False.
        color (Union[str, None], optional): table transaction special string colors.
                                            Defaults to None.

    Returns:
        Union[None, str]: the printout if 'return_str' is True
    """
    if color is None:
        color = PrintColor.NORMAL
    table_name = ' '.join(table_name.upper().split('_'))
    ...
    for _id, trans in transactions.items():
        t_string = f"{int(_id)}: "
...
def format_balances(summary: dict, return_str: bool = False) -> Union[None, str]:
    ...
    Creates a command window-friendly table of account balances
...
def format_payments(payments_dict: dict, return_str: bool = False) -> Union[None, str]:
    ...
    Creates a command window-friendly table of settle up payments

    Args:
        payments_dict (dict): payments data
        return_str (bool, optional): if the print object should be returned. Defaults to False.
